...
const Navbar = () => {
  ...
  return <header className={cn('fixed w-full z-50 transition-all duration-300', isScrolled ? 'py-4 bg-podcast-magenta shadow-md' : 'py-7 bg-podcast-magenta')} style={{
    minHeight: 50
  }}>
      <div className="container flex items-center justify-between">
        <a href="/" className="flex items-center">
          <img src="/lovable-uploads/ab5fd662-cd0e-400c-9b54-6ea329252036.png" alt="אחותי היפה" className="h-10" />
        </a>
        {/* Desktop Navigation */}
        ...
        {/* Mobile Menu Button */}
        <button className="md:hidden text-2xl text-white" onClick={toggleMenu} aria-label={menuOpen ? "סגור תפריט" : "פתח תפריט"}>
          {menuOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </div>
      {/* Mobile Navigation */}
      <nav className={cn('fixed inset-0 bg-podcast-magenta pt-24 px-6 flex flex-col items-center space-y-8 md:hidden transition-transform duration-300 ease-in-out', menuOpen ? 'translate-x-0' : 'translate-x-full')}>
        <button className="absolute top-6 right-6 text-white" onClick={toggleMenu} aria-label="סגור תפריט">
          <X size={24} />
        </button>
